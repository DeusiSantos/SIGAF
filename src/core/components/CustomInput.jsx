...
const CustomInput = ({
      ...
      <>
        <div className="grid grid-cols-7 gap-1">
          {weekdays.map((day, index) => (
            <div
              key={`weekday-${index}`}
              className="text-center text-gray-500 text-xs py-1"
            >
              {day}
            </div>
          ))}
          ...
          {days.map((day, index) => (
            <button
              ...
              className={`
                ...
                ${
                  !isSelectedDate(day) && hoveredDay === day
                    ? "bg-blue-100"
                    : ""
                }
                ${
                  !isSelectedDate(day) && !isDisabledDate(day)
          ...
          return (
            <button
              ...
              className={`
                ...
                ${
                  isCurrentYear(year) && !isSelectedYear(year)
                    ? "font-bold"
                    : ""
                }
            ...
            filteredOptions.map((option, idx) => (
              ...
              >
                {type === "multiselect" ? (
                  ...
                  <span
                    className={`w-5 h-5 mr-2.5 rounded-md flex items-center justify-center transition-colors ${
                      isItemSelected(option)
    ...
    switch (type) {
      ...
      case "select":
        return (
          <div className="relative">
            {(iconStart || !iconStart) && (
              <span className={getIconClasses("start")}>
                {iconStart || <ChevronDown size={18} />}
              </span>
            )}
            <button
              ref={inputRef}
            ...
            {!iconStart && (
              <span
                className={`${getIconClasses("end")} transition-transform ${
      ...
      case "textarea":
        return (
          <div className="relative">
            <span className={`${getIconClasses("start")} top-3 transform-none`}>
              {getIconByType()}
            </span>
            <textarea
              ref={inputRef}
  ...
  return (
    <div className="mb-4">
      ...
      <div className={getContainerClasses()}>{renderInput()}</div>
