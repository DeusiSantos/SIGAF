...
const cooperativasAdaptadas = [
    {
        id: 1,
        nomeCooperativa: "Cooperativa Agrícola Esperança",
        sigla: "CAE001",
        nif: "1234567890",
...
const EntidadesAssociativasGestao = () => {
    ...
    const [cooperativaToDelete, setCooperativaToDelete] = useState(null);

    // Integração com hook useCooperativas
    const {
        cooperativas,
        loading,
        error,
    ...
    const filteredEscolas = cooperativas.filter(cooperativa => {
        ...
        const matchesTipo = !selectedTipo || (cooperativa.atividades && cooperativa.atividades.includes(selectedTipo));
    ...
    // Navegação (simulada)
    const handleViewEscola = (cooperativaId) => {
        navigate(`/GerenciaRNPA/gestao-cooperativa/cooperativa/visualizar/${cooperativaId}`);
    };


    const handleTransferencia = (cooperativaId) => {
        // Navegar para a rota de cadastro de produção passando o ID
        navigate(`/GerenciaRNPA/entidades-associativas/cadastro-producao-cooperativa/${cooperativaId}`);
    };
    ...
    const openDeleteModal = (cooperativaId) => {
        setCooperativaToDelete(cooperativaId);
    ...
    const closeDeleteModal = () => {
        ...
        setCooperativaToDelete(null);
    };

    // Função para deletar cooperativa após confirmação
    const handleConfirmDelete = async () => {
        if (!cooperativaToDelete) return;
        try {
            await deleteCooperativa(cooperativaToDelete);
            showToast('success', 'Excluído', 'Cooperativa excluída com sucesso!');
        } catch (erro) {
            showToast('error', 'Erro', 'Erro ao excluir cooperativa.');
            console(erro)
        } finally {
            closeDeleteModal();
        }
   ...
   { /* const getTipoEnsinoLabel = (tipo) => {
    ...
    const uniqueRegions = [...new Set(cooperativas.map(escola => escola.provincia))].filter(Boolean);

    // Modal de confirmação visual
    const DeleteConfirmModal = () => {
        if (!showDeleteModal) return null;
        const cooperativa = cooperativas.find(c => c.id === cooperativaToDelete);
        return (
                ...
                <div className="bg-white rounded-xl shadow-md p-6">
                    <div className="flex items-center">
                        ...
                        <div className="ml-4">
                            ...
                            <p className="text-2xl font-bold text-gray-900">{cooperativas.length}</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="flex items-center">
                        ...
                        <div className="ml-4">
                            ...
                            <p className="text-2xl font-bold text-gray-900">
                                {cooperativas.filter(c => c.status === 'ATIVO').length}
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div className="w-full bg-white rounded-xl shadow-md overflow-visible z-10">
                ...
                <div className="bg-gradient-to-r from-blue-700 to-blue-500 p-6 text-white rounded-t-xl">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center space-y-4 md:space-y-0">
                        <div>
                            <h1 className="text-2xl font-bold">Gestão de Entidades Cooperativas</h1>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="space-y-2">
                                            <div className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                                {cooperativa.atividades.slice(0, 1).map(a => a.replace(/[-_]/g, ' ')).join(', ')}
                                            </div>
                            ...
                            <div className="flex items-start">
                                ...
                                <div className="flex-1 ml-4">
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <h3 className="text-sm font-semibold text-gray-900">{escola.nomeCooperativa}</h3>
...
export default EntidadesAssociativasGestao;