...
const GestaoAssociacaoRurais = () => {
    ...
    const filteredEscolas = associacoesRurais.filter(cooperativa => {
        const matchesSearch = cooperativa.nomeCooperativa.toLowerCase().includes(searchTerm.toLowerCase()) ||
            cooperativa.presidente.toLowerCase().includes(searchTerm.toLowerCase()) ||
            cooperativa.email.toLowerCase().includes(searchTerm.toLowerCase());
        const matchesRegion = !selectedRegion || cooperativa.provincia === selectedRegion;
        const matchesTipo = !selectedTipo || cooperativa.atividades.includes(selectedTipo);
        const matchesStatus = !selectedStatus || cooperativa.status === selectedStatus;
    ...
    const uniqueRegions = [...new Set(associacoesRurais.map(escola => escola.adminRegionalId))].filter(Boolean);
    ...
    // Função para deletar associação após confirmação
    const handleConfirmDelete = async () => {
        if (!associacaoToDelete) return;
        try {
            await deleteAssociacaoRural(associacaoToDelete);
            showToast('success', 'Excluído', 'Associação excluída com sucesso!');
        } catch (err) {
            showToast('error', 'Erro', 'Erro ao excluir associação.');
            console(err)
        } finally {
            closeDeleteModal();
        }
    ...
    const DeleteConfirmModal = () => {
        ...
        const associacao = associacoesRurais.find(c => c.id === associacaoToDelete);
        return (
            ...
            <div className="mt-6 mb-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-white rounded-xl shadow-md p-6">
                    <div className="flex items-center">
                        ...
                        <div className="ml-4">
                            <p className="text-sm font-medium text-gray-500">Total de Associação</p>
                            <p className="text-2xl font-bold text-gray-900">{associacoesRurais.length}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-xl shadow-md p-6">
                    <div className="flex items-center">
                        ...
                        <div className="ml-4">
                            <p className="text-sm font-medium text-gray-500">Associações Activas</p>
                            <p className="text-2xl font-bold text-gray-900">
                                {associacoesRurais.filter(c => c.status === 'ATIVO').length}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-xl shadow-md p-6">
                    <div className="flex items-center">
                        ...
                        <div className="ml-4">
                            <p className="text-sm font-medium text-gray-500">Área Total (ha)</p>
                            <p className="text-2xl font-bold text-gray-900">
                                0
                            </p>
                        </div>
            ...
            <div className="w-full bg-white rounded-xl shadow-md overflow-visible z-10">
                ...
                <div className="bg-gradient-to-r from-blue-700 to-blue-500 p-6 text-white">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center space-y-4 md:space-y-0">
                        <div>
                            <h1 className="text-2xl font-bold">Gestão de Associações Rurais</h1>
                ...
                <div className="hidden md:block overflow-visible" style={{ maxHeight: contentHeight }}>
                    <table className="w-full border-collapse">
                        ...
                        <tbody className="divide-y divide-gray-200 bg-white text-start">
                            {getCurrentItems().map((cooperativa) => (
                                <tr key={cooperativa.id} className="hover:bg-blue-50 transition-colors">
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="flex items-start">
                                            ...
                                            <div className="ml-4">
                                                ...
                                                <div className="text-xs text-gray-500">Pres.: {cooperativa.presidente}</div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="space-y-2">
                                            <div className="flex items-center text-xs text-gray-700">
                                                ...
                                                {cooperativa.municipio}, {cooperativa.provincia}
                                            </div>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="space-y-2">
                                            <div className="flex items-center text-xs text-gray-700">
                                                ...
                                                {cooperativa.numeroMembros} membros
                                            </div>
                                            <div className="flex items-center text-xs text-gray-700">
                                                <Building className="w-4 h-4 mr-2 text-blue-500" />
                                                {cooperativa.areaTotal} hectares
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="flex items-center justify-center space-x-1">
                                            <button
                                                onClick={() => handleViewEscola(cooperativa.id)}
                                ...
                                <div className="flex-1 ml-4">
                                    ...
                                    <div className="mt-3 grid grid-cols-2 gap-2">
                                        ...
                                        <div className="flex items-center text-xs text-gray-700">
                                            ...
                                            {escola.capacidadeAlunos} Membros
                                        </div>
                                        <div className="flex items-center text-xs text-gray-700">
                                    ...
                                    <div className="mt-3 flex justify-between items-center">
                                        ...
                                        <div className="flex items-center space-x-3">
                                            <div className="flex items-center">
                                                <span className={`w-2.5 h-2.5 rounded-full mr-1.5 ${escola.estado === 'ACTIVO' ? 'bg-green-500' : 'bg-gray-400'
                                                    }`}></span>
                                                <span className={`text-xs font-medium ${escola.estado === 'ACTIVO' ? 'text-green-600' : 'text-gray-500'
                                                    }`}>
                                                    {escola.estado}
                                                </span>
                                            </div>
                                            <button
                                                onClick={() => toggleStatus(escola.id, escola.estado)}
                                                className={`relative inline-flex h-5 w-10 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${escola.estado === 'ACTIVO' ? 'bg-green-500' : 'bg-gray-200'
                                                    }`}
                                            >
                                                <span
                                                    className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${escola.estado === 'ACTIVO' ? 'translate-x-5' : 'translate-x-0'
                                                        }`}
                                                />
                                            </button>
...
export default GestaoAssociacaoRurais;