...
import {
  useProdutores,
  useProdutoresAprovados,
} from "../../../hooks/useRnpaData";
...
const ProdutorAvatar = ({
  ...
  if (imageLoading && showLoadingSpinner) {
    return (
      <div
        className={`${size} rounded-full bg-gray-200 flex items-center justify-center shadow-sm animate-pulse`}
      >
  ...
  if (imageUrl && !imageError) {
    return (
      <div
        className={`${size} rounded-full overflow-hidden shadow-sm border-2 border-white`}
      >
        <img
          src={imageUrl}
  ...
  return (
    <div
      className={`${size} rounded-full bg-gradient-to-r from-blue-400 to-blue-600 flex items-center justify-center text-white font-bold ${textSize} shadow-sm`}
    >
...
const mapApiDataToProdutor = (apiData) => {
  ...
  return apiData.map((item) => {
    ...
    const mapAtividades = (atividadesString) => {
      ...
      atividadesComVirgula.forEach((atividade) => {
        ...
        if (trimmed.includes(" ")) {
          ...
          const subAtividades = trimmed
            .split(" ")
            .filter((sub) => sub.trim() !== "");
          subAtividades.forEach((subAtividade) => {
            const finalAtividade = subAtividade
              .trim()
              .toUpperCase()
              .replace(/_/g, " ");
    ...
    return {
      ...
      codigoRNPA: `RNPA${new Date(
        item.registration_date || "2025-01-01"
      ).getFullYear()}${item._id?.toString().slice(-3)}`,
      nome:
        item.beneficiary_name ||
        `${item.nome_produtor || ""} ${item.nome_meio_produtor || ""} ${
          item.sobrenome_produtor || ""
        }`.trim(),
      ...
      numeroAnimais:
        parseInt(item.numero_aves) ||
        parseInt(item.numero_ovelhas) ||
        parseInt(item.numero_vacas) ||
        0,
      statusProcesso: getStatus(item),
      dataSubmissao:
        item.registration_date || item._submission_time?.split("T")[0],
      inquiridor: `${item.codigo_inquiridor || "N/A"} - ${
        item.nome_inquiridor || "N/A"
      } ${item.sobrenome_inquiridor || ""}`.trim(),
...
const ProdutoresGestao = () => {
  ...
  const { produtor: produtoresAprovados, loading: loadingAprovados } =
    useProdutoresAprovados();
  ...
  const handleViewProdutor = (produtorId) => {
    navigate(
      `/GerenciaRNPA/gestao-agricultores/produtores/visualizar/${produtorId}`
    );
  };

  const handleHistoricoProducao = (produtorId) => {
    navigate(
      `/GerenciaRNPA/gestao-agricultores/produtores/historico-producao/${produtorId}`
    );
  };
  ...
  const handleGerarCartao = (produtorId) => {
    navigate(
      `/GerenciaRNPA/gestao-agricultores/produtores/gerar-cartao/${produtorId}`
    );
  };
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
        <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-sm flex flex-col items-center">
          ...
          <p className="text-gray-600 text-center text-sm mb-4">
            Tem certeza que deseja excluir o produtor{" "}
            <span className="font-semibold text-red-600">
              {produtor?.nome || "Selecionado"}
            </span>
            ?<br />
            Esta ação não pode ser desfeita. Todos os dados do produtor serão
    ...
    setLocalProdutores((prevProdutores) =>
      prevProdutores.map((produtor) =>
        produtor.id === produtorId
          ? { ...produtor, statusProcesso: newStatus }
          : produtor
      )
    );
  ...
  const filteredProdutores = localProdutores.filter((produtor) => {
    ...
    const matchesStatus =
      !selectedStatus || produtor.statusProcesso === selectedStatus;
    const matchesProvince =
      !selectedProvince || produtor.provincia === selectedProvince;
  ...
  const getActionItems = (produtor) => {
    const baseActions = [
      {
        ...
        action: handleHistoricoProducao,
      },
    ...
    if (produtor.statusProcesso === "APROVADO") {
      baseActions.push({
        label: "Gerar Cartão RNPA",
        icon: <CreditCard size={16} />,
        action: handleGerarCartao,
      });
    }
  ...
  const Toast = () => {
    ...
    return (
      <div
        className={`fixed top-4 right-4 p-4 rounded-lg shadow-lg max-w-md z-50 ${bgColor} animate-fadeIn`}
      >
  ...
  const StatusMenu = ({ produtor }) => {
    ...
    const currentStatus = statusOptions.find(
      (s) => s.value === produtor.statusProcesso
    );
  ...
  const totalPendentes = localProdutores.filter(
    (p) => p.statusProcesso === "PENDENTE"
  ).length;
  const totalAprovados = localProdutores.filter(
    (p) => p.statusProcesso === "APROVADO"
  ).length;
  const totalRejeitados = localProdutores.filter(
    (p) => p.statusProcesso === "REJEITADO"
  ).length;
  ...
  return (
    <div>
      ...
      >
        ...
        <div
          className="w-full bg-white rounded-xl shadow-md overflow-auto"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div>
                <CustomInput
                  ...
                  value={
                    selectedProvince
                      ? { label: selectedProvince, value: selectedProvince }
                      : null
                  }
                  options={[
                        ...
                        <div className="flex items-start text-start">
                          ...
                          <div className="ml-4">
                            ...
                            <div className="text-xs text-gray-500 mt-1">
                              Código: {produtor.codigoRNPA}
                            </div>
                            <div className="text-xs text-gray-500">
                              BI: {produtor.numeroBI}
                            </div>
                            <div className="text-xs text-gray-500">
                              {calculateAge(produtor.dataNascimento)}
                            </div>
                          </div>
                        </div>
                      </td>
                  ...
                  <div className="flex items-start">
                    ...
                    <div className="flex-1 ml-4">
                      ...
                      <div className="mt-3 space-y-2">
                        ...
                        <div className="text-xs text-gray-600">
                          Área: {produtor.areaTotalHa} ha | Animais:{" "}
                          {produtor.numeroAnimais}
                        </div>
                      </div>
