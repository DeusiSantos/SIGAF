...
import provinciasData from "../../../../../core/components/Provincias.json"; // Supondo que você tenha um arquivo JSON com os dados das províncias
...
const inquiridoresData = [
  {
    codigo: "INQ001",
    nomeCompleto: "João Manuel Santos",
    nomeDoMeio: "Manuel",
    sobrenome: "Santos",
  },
  {
    codigo: "INQ002",
    nomeCompleto: "Maria Teresa Silva",
    nomeDoMeio: "Teresa",
    sobrenome: "Silva",
  },
  {
    codigo: "INQ003",
    nomeCompleto: "António Carlos Ferreira",
    nomeDoMeio: "Carlos",
    sobrenome: "Ferreira",
  },
];
          ...
          <Marker position={center} icon={defaultIcon}>
            <Popup>
              <div className="text-center">
                <strong>Localização do Produtor</strong>
                <br />
...
const CadastroProdutor = () => {
  ...
  const initialState = {
    ...
    tiposAtividades: [{ label: "Agricultura", value: "AGRICULTURA" }],
    outroTipoAtividade: "",
    acessoTerras: false,
    ...
    idiomasFalados: [{ label: "Português", value: "PORTUGUES" }],
  };
      ...
      if (response.status === 200 && data.code === 200 && data.data) {
        ...
        if (!nomeProdutor && !sobrenomeProdutor) {
          showToast(
            "warn",
            "Dados incompletos",
            "Os dados do BI não contêm nome válido."
          );
          return;
        }
        ...
        if (biInfo.birth_province_name) {
          const nomeProvinciaAPI = biInfo.birth_province_name
            .toLowerCase()
            .trim();
        ...
        if (biInfo.birth_date) {
          try {
            dataNascimentoFormatada = new Date(biInfo.birth_date)
              .toISOString()
              .split("T")[0];
          } catch (error) {
        ...
        showToast(
          "success",
          "BI Consultado",
          "Dados do produtor preenchidos automaticamente!"
        );
      } else {
        ...
        if (data.code === 404) {
          showToast(
            "warn",
            "BI não encontrado",
            "Não foi possível encontrar dados para este BI. Preencha manualmente."
          );
        } else {
          showToast(
            "warn",
            "BI inválido",
            `Este BI não retornou dados válidos. Código: ${data.code}`
          );
        }
      }
    } catch (error) {
      ...
      if (error.response) {
        ...
        if (status === 401) {
          showToast(
            ...
            "Credenciais inválidas para consulta do BI."
          );
        } else if (status === 403) {
          showToast(
            "error",
            "Acesso Negado",
            "Sem permissão para consultar este BI."
          );
        } else if (status === 404) {
          showToast(
            "warn",
            "BI não encontrado",
            "Este número de BI não foi encontrado na base de dados."
          );
        } else if (status >= 500) {
          showToast(
            "error",
            "Erro do Servidor",
            "Servidor da consulta de BI indisponível. Tente novamente mais tarde."
          );
        } else {
          showToast(
            "error",
            "Erro na Consulta",
            `Erro ${status}: ${data?.message || "Erro desconhecido"}`
          );
        }
        ...
        showToast(
          "error",
          "Erro Interno",
          "Erro interno na consulta do BI. Tente novamente."
        );
      }
    } finally {
  ...
  const handleInputChange = (field, value) => {
    ...
    if (field === "codigo_inquiridor") {
      const inquiridor = inquiridoresData.find(
        (inq) => inq.codigo === (value?.value || value)
      );
      if (inquiridor) {
        setFormData((prev) => ({
          ...prev,
    ...
    if (
      ...
      (field.includes("feminino") || field.includes("masculino"))
    ) {
      ...
      if (newTotal > (formData.totalMembros || 1)) {
        showToast(
          "warn",
          "Limite excedido",
          `O total de membros distribuídos não pode exceder ${formData.totalMembros} pessoas.`
        );
        return;
      }
    }
  ...
  const validateCurrentStep = () => {
    ...
    switch (activeIndex) {
      case 0: // Inquiridor
        if (!formData.codigo_inquiridor)
          newErrors.codigo_inquiridor = "Campo obrigatório";
      ...
      case 1: // Localização
        ...
        if (!formData.bairroAldeia)
          newErrors.bairroAldeia = "Campo obrigatório";
        if (!formData.localResidencia)
          newErrors.localResidencia = "Campo obrigatório";
        console.log("❌ Erros encontrados:", newErrors);
        break;

      case 2: // Produtor
        if (!formData.nomeProdutor)
          newErrors.nomeProdutor = "Campo obrigatório";
        if (!formData.sobrenomeProdutor)
          newErrors.sobrenomeProdutor = "Campo obrigatório";
        if (!formData.sexoProdutor)
          newErrors.sexoProdutor = "Campo obrigatório";
        if (!formData.tipoDocumento)
          newErrors.tipoDocumento = "Campo obrigatório";
        if (
          !formData.lugarNascimento ||
          (typeof formData.lugarNascimento === "object" &&
            !formData.lugarNascimento.value)
        ) {
          newErrors.lugarNascimento = "Campo obrigatório";
        }
        if (
          !formData.estadoCivil ||
          (typeof formData.estadoCivil === "object" &&
            !formData.estadoCivil.value)
        ) {
        ...
        if (!formData.telefoneProdutor)
          newErrors.telefoneProdutor = "Campo obrigatório";

        if (!formData.dataNascimento)
          newErrors.dataNascimento = "Campo obrigatório";
        if (
          formData.dataNascimento &&
          calculateAge(formData.dataNascimento) < 18
        ) {
      ...
      case 3: // Agregado
        ...
        if (
          formData.chefeAgregado === null ||
          formData.chefeAgregado === undefined
        ) {
      ...
      case 4: // Atividades
        if (
          ...
          formData.tiposAtividades.length === 0
        ) {
      ...
      case 6: // Crédito & Bens
        ...
        if (
          formData.beneficiadoCredito === null ||
          formData.beneficiadoCredito === undefined
        ) {
  ...
  const renderStepContent = (index) => {
    switch (index) {
          ...
          <div className="max-w-full mx-auto">
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl p-6 mb-8 border border-blue-100">
              <div className="flex items-center space-x-3 mb-3">
                ...
                <h3 className="text-xl font-bold text-gray-800">
                  Identificação do Inquiridor
                </h3>
              </div>
              <p className="text-gray-600">
                Selecione o código do inquiridor responsável por este cadastro.
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <CustomInput
                ...
                onChange={(value) =>
                  handleInputChange("codigo_inquiridor", value)
                }
                required
        ...
        return (
          <div className="max-w-full mx-auto">
            <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-2xl p-6 mb-8 border border-green-100">
              ...
              <p className="text-gray-600">
                Informe a localização geográfica do produtor e use o mapa para
                confirmar as coordenadas.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
              ...
              <CustomInput
                ...
                onChange={(value) =>
                  handleInputChange("localResidencia", value)
                }
          ...
          <div className="max-w-full mx-auto">
            <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-2xl p-6 mb-8 border border-purple-100">
              ...
              <p className="text-gray-600">
                ...
                <span className="font-semibold">Idade mínima: 18 anos</span>
                {formData.tipoDocumento?.value === "BI" && (
                  <span className="ml-2 text-blue-600 font-semibold">
                    Digite o BI para consulta automática dos dados!
                  </span>
                )}
              </p>
            </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    ...
                    <p className="text-green-700 text-sm font-medium">
                      Dados preenchidos automaticamente através da consulta do
                      BI. Verifique e ajuste se necessário.
                    </p>
                  </div>
                  <button
                    type="button"
                    onClick={() => {
                      ...
                      showToast(
                        "info",
                        "Dados limpos",
                        "Campos limpos. Preencha manualmente."
                      );
                    }}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              {shouldShowDocumentName() && (
                <CustomInput
                  ...
                  onChange={(value) =>
                    handleInputChange("nomeOutroDocumento", value)
                  }
                  required
              ...
              <CustomInput
                ...
                onChange={(value) =>
                  handleInputChange("sobrenomeProdutor", value)
                }
              ...
              <CustomInput
                ...
                onChange={(value) =>
                  handleInputChange("lugarNascimento", value)
                }
              ...
              <CustomInput
                ...
                onChange={(value) =>
                  handleInputChange("telefoneProdutor", value)
                }
                required
              ...
              <CustomInput
                ...
                onChange={(value) =>
                  handleInputChange("tipoOrganizacao", value)
                }
                placeholder="Selecione o tipo"
                iconStart={<Building size={18} />}
              />
              ...
              {formData.tipoOrganizacao?.value === "OUTRO" && (
                <CustomInput
                  ...
                  onChange={(value) =>
                    handleInputChange("outroTipoOrganizacao", value)
                  }
              ...
              {!formData.telefonePropriedade && (
                <CustomInput
                  ...
                  onChange={(value) =>
                    handleInputChange("proprietarioTelefone", value)
                  }
              ...
              <CustomInput
                ...
                onChange={(value) =>
                  handleInputChange("nivelEscolaridade", value)
                }
              ...
              {formData.nivelEscolaridade?.value === "OUTRO" && (
                <CustomInput
                  ...
                  onChange={(value) =>
                    handleInputChange("outroNivelEscolaridade", value)
                  }
              ...
              {(formData.deficiencia === true ||
                formData.deficiencia?.value === true) && (
                <>
                  <CustomInput
                    type="select"
                    ...
                    onChange={(value) =>
                      handleInputChange("tipoDeficiencia", value)
                    }
                    placeholder="Selecione o tipo"
                  ...
                  {formData.tipoDeficiencia?.value === "OUTRO" && (
                    <CustomInput
                      ...
                      onChange={(value) =>
                        handleInputChange("outraDeficiencia", value)
                      }
              ...
              <div className="mt-8 bg-blue-50 rounded-2xl p-6 border border-blue-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="text-gray-800">
                      {biData.birth_date
                        ? new Date(biData.birth_date).toLocaleDateString(
                            "pt-BR"
                          )
                        : "N/A"}
                    </p>
                  ...
                  <div>
                    ...
                    <p className="text-gray-800">
                      {biData.birth_date
                        ? `${calculateAge(biData.birth_date)} anos`
                        : "N/A"}
                    </p>
                  </div>
                  <div>
                    ...
                    <p className="text-gray-800">
                      {biData.marital_status_name || "N/A"}
                    </p>
                  </div>
                  <div>
                    <span className="font-medium text-gray-600">
                    ...
                    <p className="text-gray-800">
                      {biData.birth_province_name || "N/A"}
                    </p>
                  </div>
                  <div>
                    <span className="font-medium text-gray-600">
                    ...
                    <p className="text-gray-800">
                      {biData.issue_date
                        ? new Date(biData.issue_date).toLocaleDateString(
                            "pt-BR"
                          )
                        : "N/A"}
                    </p>
                  ...
                  <div>
                    ...
                    <p className="text-gray-800">
                      {biData.expiry_date
                        ? new Date(biData.expiry_date).toLocaleDateString(
                            "pt-BR"
                          )
                        : "N/A"}
                    </p>
              ...
              <div className="bg-white rounded-2xl border border-gray-200 p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <CustomInput
                    ...
                    onChange={(value) =>
                      handleInputChange("chefeAgregado", value)
                    }
                    placeholder="Selecione uma opção"
                    iconStart={<Users size={18} />}
                  />
                  ...
                  <CustomInput
                    ...
                    onChange={(value) =>
                      handleInputChange("totalMembros", parseInt(value) || 1)
                    }
                    required
                ...
                {(formData.chefeAgregado === false ||
                  formData.chefeAgregado?.value === false) && (
                  <div className="mt-6 p-6 bg-yellow-50 rounded-xl border border-yellow-200">
                    <h5 className="text-md font-semibold text-yellow-800 mb-4">
                      Informações do Chefe do Agregado
                    </h5>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      <CustomInput
                        type="text"
                        ...
                        onChange={(value) =>
                          handleInputChange("nomeChefeAgregado", value)
                        }
                      ...
                      <CustomInput
                        ...
                        onChange={(value) =>
                          handleInputChange("nomeDoMeioChefe", value)
                        }
                        placeholder="Nome do meio (opcional)"
                        iconStart={<User size={18} />}
                      />
                      ...
                      <CustomInput
                        ...
                        onChange={(value) =>
                          handleInputChange("sobrenomeChefe", value)
                        }
                        placeholder="Sobrenome"
                        iconStart={<User size={18} />}
                      />
                      ...
                      <CustomInput
                        ...
                        onChange={(value) =>
                          handleInputChange("sexoChefe", value)
                        }
                      ...
                      <CustomInput
                        ...
                        options={[
                          ...
                          {
                            label: "Sem relação de parentesco",
                            value: "SEM_RELACAO",
                          },
                          { label: "Não sabe", value: "NAO_SABE" },
                        ]}
                        onChange={(value) =>
                          handleInputChange("relacaoChefe", value)
                        }
              ...
              <div className="bg-white rounded-2xl border border-gray-200 p-6">
                ...
                <div className="mb-4 p-4 bg-blue-50 rounded-lg">
                  <p className="text-blue-700 text-sm font-medium flex items-center">
                    ...
                    Total definido: {formData.totalMembros || 1} membros | Total
                    distribuído: {getTotalMembrosDistribuidos()} membros |
                    Restante:{" "}
                    {(formData.totalMembros || 1) -
                    ...
                    <div className="text-center">
                      <span
                        className={`block font-semibold text-lg ${
                          getTotalMembrosDistribuidos() > formData.totalMembros
                            ? "text-red-600"
                            : "text-green-600"
                        }`}
                      >
          ...
          <div className="max-w-full mx-auto">
            <div className="bg-gradient-to-r from-green-50 to-teal-50 rounded-2xl p-6 mb-8 border border-green-100">
              <div className="flex items-center space-x-3 mb-3">
                ...
                <h3 className="text-xl font-bold text-gray-800">
                  Actividades e Activos Agrícolas
                </h3>
              </div>
              <p className="text-gray-600">
                Informações sobre as actividades praticadas, terras cultiváveis
            ...
            <div className="space-y-8">
              <div className="bg-white rounded-2xl border border-gray-200 p-6">
                ...
                <CustomInput
                  ...
                  onChange={(value) =>
                    handleInputChange("tiposAtividades", value)
                  }
                  errorMessage={errors.tiposAtividades}
                />

                {formData.tiposAtividades &&
                  formData.tiposAtividades.includes("OUTRO") && (
                    <div className="mt-4">
                      <CustomInput
                        type="text"
                        ...
                        onChange={(value) =>
                          handleInputChange("outroTipoAtividade", value)
                        }
              ...
              <div className="bg-white rounded-2xl border border-gray-200 p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  <CustomInput
                    ...
                    onChange={(value) =>
                      handleInputChange("acessoTerras", value)
                    }
                    placeholder="Selecione uma opção"
                  />

                  {(formData.acessoTerras === true ||
                    formData.acessoTerras?.value === true) && (
                    <>
                      <CustomInput
                        type="select"
                        ...
                        onChange={(value) =>
                          handleInputChange("proprietarioTerra", value)
                        }
                        placeholder="Selecione uma opção"
                      />

                      {(formData.proprietarioTerra === true ||
                        formData.proprietarioTerra?.value === true) && (
                        <>
                          <CustomInput
                            type="select"
                            ...
                            onChange={(value) =>
                              handleInputChange("tituloConcessao", value)
                            }
                            placeholder="Selecione uma opção"
                          />

                          {(formData.tituloConcessao === true ||
                            formData.tituloConcessao?.value === true) && (
                            <CustomInput
                              type="select"
                              label="Tipo de Documento"
                              ...
                              options={[
                                {
                                  label: "Direito consuetudinário",
                                  value: "CONSUETUDINARIO",
                                },
                                { label: "Formalizado", value: "FORMALIZADO" },
                              ]}
                              onChange={(value) =>
                                handleInputChange("tipoTitulo", value)
                              }
                              placeholder="Selecione o tipo"
                            />
                          )}
                  ...
                  <CustomInput
                    ...
                    onChange={(value) =>
                      handleInputChange("areaExplorada", parseFloat(value) || 0)
                    }
                    placeholder="0.0"
                    step="0.1"
                    min="0"
                  ...
                  <CustomInput
                    ...
                    onChange={(value) =>
                      handleInputChange("areaAgricola", parseFloat(value) || 0)
                    }
                    placeholder="0.0"
                    step="0.1"
                    min="0"
                  ...
                  <CustomInput
                    ...
                    onChange={(value) =>
                      handleInputChange("areaPecuaria", parseFloat(value) || 0)
                    }
                    placeholder="0.0"
                    step="0.1"
                    min="0"
                  ...
                  <CustomInput
                    ...
                    onChange={(value) =>
                      handleInputChange("areaFlorestal", parseFloat(value) || 0)
                    }
                    placeholder="0.0"
                    step="0.1"
                    min="0"
                  ...
                  <CustomInput
                    ...
                    onChange={(value) =>
                      handleInputChange("tecnologiaAgricola", value)
                    }
                    placeholder="Selecione o tipo"
                  />
                </div>
                ...
                <div className="bg-white rounded-2xl border border-gray-200 p-6">
                  ...
                  <div className="mb-6">
                    <CustomInput
                      ...
                      onChange={(value) =>
                        handleInputChange("culturasPrincipais", value)
                      }
                    />

                    {formData.culturasPrincipais &&
                      formData.culturasPrincipais.includes("OUTRO") && (
                        <div className="mt-4">
                          <CustomInput
                            type="text"
                            ...
                            onChange={(value) =>
                              handleInputChange("outraCultura", value)
                            }
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <CustomInput
                      ...
                      onChange={(value) =>
                        handleInputChange("tipoSementeira", value)
                      }
                      placeholder="Selecione o tipo"
                    />
                    ...
                    <CustomInput
                      ...
                      onChange={(value) =>
                        handleInputChange("usoFertilizantes", value)
                      }
                      placeholder="Selecione uma opção"
                    />
                    ...
                    <CustomInput
                      ...
                      onChange={(value) =>
                        handleInputChange("preparacaoTerra", value)
                      }
                      placeholder="Selecione o tipo"
                    />
                  </div>
                ...
                <div className="bg-white rounded-2xl border border-gray-200 p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <CustomInput
                      ...
                      onChange={(value) =>
                        handleInputChange("acessoIrrigacao", value)
                      }
                      placeholder="Selecione uma opção"
                    />

                    {(formData.acessoIrrigacao === true ||
                      formData.acessoIrrigacao?.value === true) && (
                      <>
                        <CustomInput
                          type="select"
                          ...
                          onChange={(value) =>
                            handleInputChange("sistemaIrrigacao", value)
                          }
                        ...
                        {formData.sistemaIrrigacao?.value === "OUTRO" && (
                          <CustomInput
                            ...
                            onChange={(value) =>
                              handleInputChange("outroSistemaIrrigacao", value)
                            }
                ...
                <div className="bg-orange-50 rounded-2xl p-6 border border-orange-200">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <CustomInput
                      ...
                      onChange={(value) =>
                        handleInputChange("amanhosCulturais", value)
                      }
                      placeholder="Selecione uma opção"
                    />

                    {(formData.amanhosCulturais === true ||
                      formData.amanhosCulturais?.value === true) && (
                      <div className="space-y-4">
                        <CustomInput
                          ...
                          onChange={(value) =>
                            handleInputChange("tipoAmanhos", value)
                          }
                        />

                        {formData.tipoAmanhos &&
                          formData.tipoAmanhos.includes("OUTROS") && (
                            <CustomInput
                              type="text"
                              label="Especificar Outros Amanhos"
                              value={formData.outroTipoAmanho}
                              onChange={(value) =>
                                handleInputChange("outroTipoAmanho", value)
                              }
                ...
                <div className="bg-green-50 rounded-2xl p-6 border border-green-200">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <CustomInput
                      ...
                      onChange={(value) =>
                        handleInputChange("acessoInstrumentos", value)
                      }
                      placeholder="Selecione uma opção"
                    />

                    {(formData.acessoInstrumentos === true ||
                      formData.acessoInstrumentos?.value === true) && (
                      <div className="space-y-4">
                        <CustomInput
                          type="multiselect"
                          ...
                          options={[
                            ...
                            {
                              label: "Caixas comunitárias",
                              value: "CAIXAS_COMUNITARIAS",
                            },
                            {
                              label: "Crédito agrícola",
                              value: "CREDITO_AGRICOLA",
                            },
                            { label: "Fundo próprio", value: "FUNDO_PROPRIO" },
                            {
                              label: "Mercado aberto",
                              value: "MERCADO_ABERTO",
                            },
                            { label: "Outra", value: "OUTRA" },
                          ]}
                          onChange={(value) =>
                            handleInputChange("fonteInstrumentos", value)
                          }
                        />

                        {formData.fonteInstrumentos &&
                          formData.fonteInstrumentos.includes("OUTRA") && (
                            <CustomInput
                              type="text"
            ...
            <div className="max-w-full mx-auto">
              <div className="bg-gradient-to-r from-gray-50 to-gray-100 rounded-2xl p-6 mb-8 border border-gray-200">
                ...
                <p className="text-gray-500">
                  Esta seção só é exibida se você selecionou Pecuária ou
                  Agropecuária nas atividades.
                </p>
              </div>
            </div>
          );
        ...
        return (
          <div className="max-w-full mx-auto">
            ...
            <div className="space-y-8">
                  ...
                  formData.tiposCriacao.some(
                    ...
                    <div className="mt-4">
                      <CustomInput
                        ...
                        onChange={(value) =>
                          handleInputChange("outroTipoCriacao", value)
                        }
              ...
              {formData.tiposCriacao &&
                formData.tiposCriacao.map((tipo, index) => {
                  const tipoValue =
                      ...
                      >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                          ...
                          <CustomInput
                            ...
                            onChange={(value) =>
                              handleInputChange("sistemaAvicultura", value)
                            }
                            placeholder="Selecione o sistema"
                          />
                          <CustomInput
                            ...
                            onChange={(value) =>
                              handleInputChange("objetivoAvicultura", value)
                            }
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                          ...
                          <CustomInput
                            ...
                            options={[
                              ...
                              {
                                label: "Em livre acesso",
                                value: "LIVRE_ACESSO",
                              },
                            ]}
                            onChange={(value) =>
                              handleInputChange("sistemaBovinocultura", value)
                            }
                            placeholder="Selecione o sistema"
                          />
                          <CustomInput
                            ...
                            options={[
                              { label: "Intensivo", value: "INTENSIVO" },
                              {
                                ...
                                value: "SEMI_INTENSIVO",
                              },
                              { label: "Extensivo", value: "EXTENSIVO" },
                            ]}
                            onChange={(value) =>
                              handleInputChange("tipoBovinocultura", value)
                            }
                            placeholder="Selecione o tipo"
                          />
                          <CustomInput
                            ...
                            onChange={(value) =>
                              handleInputChange("objetivoBovinos", value)
                            }
                            placeholder="Selecione o objetivo"
                          />
                          {formData.objetivoBovinos?.value === "OUTRO" && (
                            <CustomInput
                              ...
                              onChange={(value) =>
                                handleInputChange("outroObjetivoBovinos", value)
                              }
                              placeholder="Digite o objetivo"
                              iconStart={<FileText size={18} />}
                            />
                      ...
                      >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                          ...
                          <CustomInput
                            ...
                            onChange={(value) =>
                              handleInputChange("sistemaOvinocultura", value)
                            }
                            placeholder="Selecione o sistema"
                          />
                          <CustomInput
                            ...
                            onChange={(value) =>
                              handleInputChange("tipoOvinocultura", value)
                            }
                            placeholder="Selecione o tipo"
                          />
                          <CustomInput
                            ...
                            onChange={(value) =>
                              handleInputChange("objetivoOvinos", value)
                            }
                            placeholder="Selecione o objetivo"
                          />
                          {formData.objetivoOvinos?.value === "OUTRO" && (
                            <CustomInput
                              ...
                              onChange={(value) =>
                                handleInputChange("outroObjetivoOvinos", value)
                              }
                              placeholder="Digite o objetivo"
                              iconStart={<FileText size={18} />}
                            />
                      ...
                      >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                          ...
                          <CustomInput
                            ...
                            onChange={(value) =>
                              handleInputChange("sistemaCaprinocultura", value)
                            }
                            placeholder="Selecione o sistema"
                          />
                          <CustomInput
                            ...
                            onChange={(value) =>
                              handleInputChange("tipoCaprinocultura", value)
                            }
                            placeholder="Selecione o tipo"
                          />
                          <CustomInput
                            ...
                            onChange={(value) =>
                              handleInputChange("objetivoCaprinos", value)
                            }
                            placeholder="Selecione o objetivo"
                          />
                  ...
                  if (tipoValue === "SUINOCULTURA") {
                    return (
                      <div
                        key={`suinocultura-${index}`}
                        className="bg-pink-50 rounded-2xl p-6 border border-pink-200"
                      >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                          ...
                          <CustomInput
                            ...
                            onChange={(value) =>
                              handleInputChange("objetivoSuinos", value)
                            }
                            placeholder="Selecione o objetivo"
                          />
                          {formData.objetivoSuinos?.value === "OUTRO" && (
                            <CustomInput
                              ...
                              onChange={(value) =>
                                handleInputChange("outroObjetivoSuinos", value)
                              }
                              placeholder="Digite o objetivo"
                              iconStart={<FileText size={18} />}
                            />
                  ...
                  if (
                    tipoValue === "PISCICULTURA" ||
                    tipoValue === "AQUICULTURA"
                  ) {
                    const jaRenderizado = formData.tiposCriacao.some(
                      (t, i) =>
                        i < index &&
                    ...
                    return (
                      <div
                        key={`piscicultura-${index}`}
                        className="bg-cyan-50 rounded-2xl p-6 border border-cyan-200"
                      >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 text-left">
                          ...
                          <CustomInput
                            ...
                            options={[
                              {
                                label: "Piscicultura industrial",
                                value: "INDUSTRIAL",
                              },
                              {
                                label:
                                  "Pequena piscicultura informal integrada à agricultura familiar",
                                value: "INFORMAL_FAMILIAR",
                              },
                              {
                                label: "Piscicultura de subsistência",
                                value: "SUBSISTENCIA",
                              },
                            ]}
                            onChange={(value) =>
                              handleInputChange("tipoPiscicultura", value)
                            }
                            placeholder="Selecione o tipo"
                          />
                          <CustomInput
                            ...
                            onChange={(value) =>
                              handleInputChange("objetivoPiscicultura", value)
                            }
                            placeholder="Selecione o objetivo"
                          />
                      ...
                      >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                          ...
                          <CustomInput
                            ...
                            onChange={(value) =>
                              handleInputChange("objetivoCoelhos", value)
                            }
                            placeholder="Selecione o objetivo"
                          />
                          {formData.objetivoCoelhos?.value === "OUTRO" && (
                            <CustomInput
                              ...
                              onChange={(value) =>
                                handleInputChange("outroObjetivoCoelhos", value)
                              }
                              placeholder="Digite o objetivo"
                              iconStart={<FileText size={18} />}
                            />
              ...
              <div className="bg-green-50 rounded-2xl p-6 border border-green-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <CustomInput
                    ...
                    onChange={(value) =>
                      handleInputChange("acessoRacao", value)
                    }
                    placeholder="Selecione uma opção"
                  />
                  ...
                  <CustomInput
                    ...
                    onChange={(value) =>
                      handleInputChange("conhecimentoDoencas", value)
                    }
                    placeholder="Selecione uma opção"
                  />
                </div>
          ...
          <div className="max-w-full mx-auto">
            <div className="bg-gradient-to-r from-yellow-50 to-orange-50 rounded-2xl p-6 mb-8 border border-yellow-100">
              <div className="flex items-center space-x-3 mb-3">
                ...
                <h3 className="text-xl font-bold text-gray-800">
                  Crédito e Bens Patrimoniais
                </h3>
              </div>
              <p className="text-gray-600">
                Informações sobre acesso a crédito, instrumentos agrícolas e
            ...
            <div className="space-y-8">
              ...
              <div className="bg-blue-50 rounded-2xl p-6 border border-blue-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <CustomInput
                    ...
                    onChange={(value) =>
                      handleInputChange("beneficiadoCredito", value)
                    }
                    placeholder="Selecione uma opção"
                  ...
                  {(formData.beneficiadoCredito === true ||
                    formData.beneficiadoCredito?.value === true) && (
                    <div className="space-y-4">
                      <CustomInput
                        type="multiselect"
                        ...
                        onChange={(value) =>
                          handleInputChange("fontesCredito", value)
                        }
                      />

                      {formData.fontesCredito &&
                        formData.fontesCredito.includes("OUTROS") && (
                          <CustomInput
                            type="text"
                            label="Especificar Outras Fontes de Crédito"
                            value={formData.outraFonteCredito}
                            onChange={(value) =>
                              handleInputChange("outraFonteCredito", value)
                            }
              ...
              <div className="bg-purple-50 rounded-2xl p-6 border border-purple-200">
                ...
                <p className="text-purple-700 mb-6">
                  Selecione todos os bens que a família possui:
                </p>
                ...
                <CustomInput
                  ...
                  options={[
                    ...
                    {
                      label: "Motorizada 2 rodas",
                      value: "MOTORIZADA_2_RODAS",
                    },
                    {
                      label: "Motorizada 3 rodas",
                      value: "MOTORIZADA_3_RODAS",
                    },
                    ...
                    {
                      label: "Teto de chapa de zinco",
                      value: "TETO_CHAPA_ZINCO",
                    },
                    { label: "Outro", value: "OUTRO" },
                  ]}
                  onChange={(value) =>
                    handleInputChange("bensPatrimonio", value)
                  }
                />

                {formData.bensPatrimonio &&
                  formData.bensPatrimonio.includes("OUTRO") && (
                    <div className="mt-4">
                      <CustomInput
                        type="text"
                        ...
                        onChange={(value) =>
                          handleInputChange("outroBemPatrimonio", value)
                        }
        ...
        return (
          <div className="max-w-full mx-auto">
            <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-2xl p-6 mb-8 border border-indigo-100">
              ...
              <p className="text-gray-600">
                Faça o upload dos documentos necessários para completar o
                cadastro no RNPA.
              </p>
            </div>
              ...
              <div className="flex items-start">
                ...
                <div className="ml-3">
                  <p className="text-sm text-amber-700 font-medium">
                    <strong>Documentos obrigatórios:</strong> Foto biométrica e
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="relative">
                      <input
                        ...
                        accept="image/*"
                        onChange={(e) =>
                          handleFileUpload("fotoBiometrica", e.target.files[0])
                        }
                      ...
                      >
                        ...
                        >
                          {uploadedFiles.fotoBiometrica
                            ? "Foto carregada"
                            : "Carregar foto biométrica"}
                        </p>
                        {uploadedFiles.fotoBiometrica && (
                          <p className="text-xs text-blue-500 mt-1">
                            {uploadedFiles.fotoBiometrica.name}
                          </p>
                        )}
                      </label>
                    </div>
                    ...
                    <div className="relative">
                      ...
                      >
                        ...
                        >
                          {uploadedFiles.fotoNaoBiometrica
                            ? "Foto carregada"
                            : "Carregar foto opcional"}
                        </p>
                        {uploadedFiles.fotoNaoBiometrica && (
                          <p className="text-xs text-blue-500 mt-1">
                            {uploadedFiles.fotoNaoBiometrica.name}
                          </p>
                        )}
                      </label>
                    </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="relative">
                      <input
                        ...
                        onChange={(e) =>
                          handleFileUpload("documentoFrente", e.target.files[0])
                        }
                      ...
                      >
                        ...
                        >
                          {uploadedFiles.documentoFrente
                            ? "Documento carregado"
                            : "Carregar documento frente"}
                        </p>
                        {uploadedFiles.documentoFrente && (
                          <p className="text-xs text-blue-500 mt-1">
                            {uploadedFiles.documentoFrente.name}
                          </p>
                        )}
                      </label>
                    </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="relative">
                      <input
                        ...
                        accept=".pdf,.jpg,.jpeg,.png"
                        onChange={(e) =>
                          handleFileUpload("documentoVerso", e.target.files[0])
                        }
                      ...
                      >
                        ...
                        >
                          {uploadedFiles.documentoVerso
                            ? "Documento carregado"
                            : "Carregar documento verso"}
                        </p>
                        {uploadedFiles.documentoVerso && (
                          <p className="text-xs text-blue-500 mt-1">
                            {uploadedFiles.documentoVerso.name}
                          </p>
                        )}
                      </label>
                    </div>
                    ...
                    <div className="relative">
                      ...
                      >
                        ...
                        >
                          {uploadedFiles.superficieCultivada
                            ? "Foto carregada"
                            : "Carregar foto da cultura"}
                        </p>
                        {uploadedFiles.superficieCultivada && (
                          <p className="text-xs text-blue-500 mt-1">
                            {uploadedFiles.superficieCultivada.name}
                          </p>
                        )}
                      </label>
                    </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="relative">
                      <input
                        ...
                        multiple
                        onChange={(e) =>
                          handleFileUpload("fotosAnimais", e.target.files[0])
                        }
                        id="animais-upload"
                      />
                      <label
                      ...
                      >
                        ...
                        {uploadedFiles.fotosAnimais && (
                          <p className="text-xs text-blue-500 mt-1">
                            {uploadedFiles.fotosAnimais.name}
                          </p>
                        )}
                      </label>
                    </div>
              ...
              <div className="bg-green-50 rounded-2xl p-6 border border-green-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <CustomInput
                    ...
                    options={[
                      {
                        label: "Milho+Feijão+Galinha+Cabra+Vaca+Boi",
                        value: "PACOTE_1",
                      },
                      {
                        label: "Massango+Feijão+Galinha+Cabrito+Ovelha+Boi",
                        value: "PACOTE_2",
                      },
                      {
                        label: "Massambala+Feijão+Galinha+Cabrito+Ovelha+Boi",
                        value: "PACOTE_3",
                      },
                      { label: "Outro", value: "OUTRO" },
                    ]}
                  ...
                  {formData.tipoPacote?.value === "OUTRO" && (
                    <CustomInput
                      ...
                      onChange={(value) =>
                        handleInputChange("outroTipoPacote", value)
                      }
                ...
                <div className="mt-6">
                  <CustomInput
                    ...
                    onChange={(value) =>
                      handleInputChange("observacoesGerais", value)
                    }
      ...
      default:
        return (
          <div className="text-center text-gray-500">Etapa não encontrada</div>
        );
    }
  };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const apiData = {
        // Seção A: Identificação do Inquiridor
        codigo_inquiridor:
          formData.codigo_inquiridor?.value || formData.codigo_inquiridor || "",
        ...
        geo_level_6:
          formData.localResidencia?.value || formData.localResidencia || "",
        gps_coordinates: `${formData.latitudeGPS || 0},${
          formData.longitudeGPS || 0
        }`,
        ...
        beneficiary_name: `${formData.nomeProdutor || ""} ${
          formData.nomeDoMeioProdutor || ""
        } ${formData.sobrenomeProdutor || ""}`
          .trim()
          .replace(/\s+/g, " "),

        // ECA/Organização
        E_4_Fazes_parte_de_uma_cooper:
          formData.tipoOrganizacao?.value === "COOPERATIVA" ? "Sim" : "Não",
        posicao_eca: formData.posicaoECA?.value || formData.posicaoECA || "N/A", // OBRIGATÓRIO - valor padrão
        tipo_organizacao:
          formData.tipoOrganizacao?.value ||
          formData.tipoOrganizacao ||
          "NENHUMA",
        especificar_organizacao: formData.outroTipoOrganizacao || "N/A", // OBRIGATÓRIO - valor padrão

        // Dados pessoais
        beneficiary_gender:
          formData.sexoProdutor?.value || formData.sexoProdutor || "",
        tipo_documento:
          formData.tipoDocumento?.value || formData.tipoDocumento || "",
        confirmar_documento: formData.numeroDocumento || "",
        foto_documento: "", // Arquivo
        beneficiary_phone_number: formData.telefoneProdutor || "",
        confirmar_telefone:
          formData.confirmarTelefoneProdutor || formData.telefoneProdutor || "",
        ...
        lugar_nascimento:
          formData.lugarNascimento?.value || formData.lugarNascimento || "",
        estado_civil: formData.estadoCivil?.value || formData.estadoCivil || "",
        nivel_escolaridade:
          formData.nivelEscolaridade?.value || formData.nivelEscolaridade || "",
        ...
        tipo_deficiencia:
          formData.tipoDeficiencia?.value || formData.tipoDeficiencia || "",
        ...
        relacao_chefe:
          formData.relacaoChefe?.value || formData.relacaoChefe || "",
        ...
        tecnologia_agricola:
          formData.tecnologiaAgricola?.value ||
          formData.tecnologiaAgricola ||
          "",
        ...
        tipo_semanteira:
          formData.tipoSementeira?.value || formData.tipoSementeira || "",
        uso_fertilizante: formData.usoFertilizantes ? "Sim" : "Não",
        preparacao_terra:
          formData.preparacaoTerra?.value || formData.preparacaoTerra || "",

        // Irrigação
        acesso_irrigacao: formData.acessoIrrigacao ? "Sim" : "Não",
        sistema_irrigacao:
          formData.sistemaIrrigacao?.value || formData.sistemaIrrigacao || "",
        ...
        // Avicultura
        sistema_avicultura:
          formData.sistemaAvicultura?.value || formData.sistemaAvicultura || "",
        objetivo_avicultura:
          formData.objetivoAvicultura?.value ||
          formData.objetivoAvicultura ||
          "",
        ...
        // Pecuária geral
        tipo_pecuaria:
          formData.sistemaBovinocultura?.value ||
          formData.sistemaBovinocultura ||
          "",
        manejo_pecuaria:
          formData.tipoBovinocultura?.value || formData.tipoBovinocultura || "",
        ...
        objetivo_producao:
          formData.objetivoBovinos?.value || formData.objetivoBovinos || "",
        ...
        objetivo_suinocultura:
          formData.objetivoSuinos?.value || formData.objetivoSuinos || "",
        especificar_objetivo_suino: formData.outroObjetivoSuinos || "N/A", // OBRIGATÓRIO - valor padrão

        // Aquicultura
        tipo_aquicultura:
          formData.tipoPiscicultura?.value || formData.tipoPiscicultura || "",
        objetivo_aquicultura:
          formData.objetivoPiscicultura?.value ||
          formData.objetivoPiscicultura ||
          "",
        ...
        objetivo_coelho:
          formData.objetivoCoelhos?.value || formData.objetivoCoelhos || "",
        ...
        H_3_O_produtor_j_beneficiou_d: formData.beneficiadoCredito
          ? "Sim"
          : "Não",
        ...
        Especifica_outro_tip_ito_que_o_beneficiou:
          formData.outraFonteCredito || "N/A", // OBRIGATÓRIO - valor padrão
      ...
      showToast(
        "success",
        "Sucesso",
        "Produtor cadastrado no RNPA com sucesso!"
      );
      ...
      if (error.response) {
        ...
        if (error.response.status === 400) {
          ...
          if (errorData && errorData.errors) {
            ...
            const errorMessages = Object.keys(errorData.errors)
              .map((field) => `${field}: ${errorData.errors[field].join(", ")}`)
              .join("\n");
            showToast(
              "error",
              "Erro de Validação",
          ...
          const errorMessage =
            error.response.data?.message || "Erro do servidor";
          showToast(
            "error",
            "Erro do Servidor",
        ...
        showToast(
          "error",
          "Erro",
          "Erro inesperado ao cadastrar produtor. Tente novamente."
        );
      }
    }
  };
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      ...
      {toastMessage && (
        <div
          className={`fixed top-4 right-4 p-4 rounded-lg shadow-lg z-50 transition-all ${
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200">
          ...
          <div className="flex justify-between items-center p-8 pt-6 border-t border-gray-100 bg-gray-50">
            ...
            <button
              ...
              disabled={
                (isLastStep && !isAllRequiredFilesUploaded()) || loading
              }
              onClick={(e) => {
                e.preventDefault();
                if (!isLastStep) {
                  ...
                  if (isAllRequiredFilesUploaded() && validateCurrentStep()) {
                    ...
                    showToast(
                      "error",
                      "Erro",
                      "Por favor, complete todos os campos e anexe os documentos necessários."
                    );
                  }
                }
              }}
